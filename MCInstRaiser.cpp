...
void MCInstRaiser::buildCFG(MachineFunction &MF, const MCInstrAnalysis *MIA,
  ...
  std::cout << "\nbuildCFG::RaiseMCInst  BEFORE. MF.dump():\n";
  LLVM_DEBUG(MF.dump());
  MF.print(outs());
                ...
                if ((Target >= FuncStart) && (Target < FuncEnd)) {
                  ...
                  if (MIA->isConditionalBranch(prevMCInst)) {
                    if ((mcInstIndex >= FuncStart) &&
                      ...
                      std::cout << "prevMCInstTargets.push_back: " << mcInstIndex << "\n";
                    }
                  }
                }
              }
  ...
  LLVM_DEBUG(dbgs() << "\nbuildCFG ended\n");
  LLVM_DEBUG(MF.dump());
  std::cout << "Generated CFG buildCFG ended. MF.dump():\n";
  std::cout << "\n";
  MF.print(outs());
  std::cout << "MF.dump() ended. \n";
